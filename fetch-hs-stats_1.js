...
// Brand-detection rules based on email name prefixes used in your HubSpot account
const BRAND_RULES = [
...
// ── 1. Fetch every marketing email (paginated) ──────────────────────────────
async function fetchAllEmails() {
  ...
  do {
    ...
    console.log(`  Page ${page}: fetched ${res.data.results?.length ?? 0} emails (total so far: ${all.length})`);
  } while (after && page < 30); // max 3,000 emails

  return all;
}

// ── 2. Fetch statistics for one email ──────────────────────────────────────
async function fetchStats(emailId) {
  try {
    ...
    // 404 = no stats yet (draft/scheduled), 403 = scope missing
    if (e.response?.status !== 404) {
      console.warn(`  WARN: stats fetch failed for ${emailId}: ${e.response?.status} ${e.response?.data?.message || ''}`);
    }
    return null;
  }
}
...
async function main() {
  ...
  console.log(`Total emails in account: ${allEmails.length}`);
  ...
  // Fetch stats for each sent email
  ...
  for (const email of sent) {
    ...
    if (stats) {
      ...
      const hardBnc   = c.hardBounced ?? 0;
      const softBnc   = c.softBounced ?? 0;
      const unsubs    = c.unsubscribed ?? 0;
      const spam      = c.spamreport  ?? 0;
      ...
      results.push({
        ...
        unsubscribes:unsubs,
        spamReports: spam,
        // Rates (rounded to 2 dp, as percentages)
    ...
    if (processed % 50 === 0) {
      console.log(`  Progress: ${processed}/${sent.length} emails processed...`);
    }

    // Respect HubSpot rate limit: 110 req / 10 s
    await sleep(100);
  }
  ...
  // ── 3. Compute brand-level summary stats ────────────────────────────────
  const brandSummary = {};
  for (const e of results) {
    if (!brandSummary[e.brand]) {
      brandSummary[e.brand] = {
        brand: e.brand, label: e.brandLabel,
        totalSent: 0, totalDelivered: 0,
        openRates: [], clickRates: [],
        sent2026: 0, delivered2026: 0,
    ...
    if (e.delivered > 0) {
      b.openRates.push(e.openRate);
      b.clickRates.push(e.clickRate);
    }
    if (e.sendDate && e.sendDate.startsWith('2026')) {
      b.sent2026++;
      b.delivered2026 += e.delivered;
      if (e.delivered > 0) {
        b.openRates2026.push(e.openRate);
        b.clickRates2026.push(e.clickRate);
      }
    }
  }
  ...
  const brandStats = Object.values(brandSummary).map(b => ({
    ...
    avgOpenRate:    avg(b.openRates),
    avgClickRate:   avg(b.clickRates),
    sent2026:       b.sent2026,
    delivered2026:  b.delivered2026,
    avgOpenRate2026:avg(b.openRates2026),
    avgClickRate2026:avg(b.clickRates2026),
  }));
